...
### Все места
```bash
PGPASSWORD=changeme psql -h 51.250.86.178 -U nike -d audioguid -c "SELECT name, epoch FROM pois;"
```
...
```bash
# 1. Запустить сервер
cd backend && export DATABASE_URL="postgresql://nike:changeme@51.250.86.178:30101/audioguid?sslmode=disable" && go run cmd/server/main.go &
